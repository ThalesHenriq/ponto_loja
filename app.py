        ...
        with st.expander("📊 Relatórios"):
            ...
            if not df.empty:
                ...
                def gerar_espelho_pdf(esp, nome_empresa, funcionario, data_inicio, data_fim):
                    ...
                    # Cálculo seguro do total de horas
                    def horas_para_decimal(texto):
                        if texto == "Incompleto" or pd.isna(texto):
                            return 0.0
                        try:
                            h = int(texto.split('h')[0].strip())
                            m = int(texto.split('min')[0].split()[-1].strip())
                            return h + m / 60.0
                        except:
                            return 0.0

                    total_horas = esp['Total Dia'].apply(horas_para_decimal).sum()
